...
class ScriptEval:
        ...
        if header.params:
            ...
            for request_rows in request:
                ...
                for row in request_rows.rows:
                    ...
                    q = q._append(params, ignore_index=True)
    ...
    def evaluate(self, context, script, ret_type, q):
        ...
        # default settings for dialog between ollama and Python. Adapt to local needs if required
        ...
        # Script is json document settings to model
        p = json.loads(script)
        rt = p.get("RequestType","none")

        if rt == "ollama":
            ollama_settings = p.get("ollama",default_ollama_settings)
            ollama_host = ollama_settings.get("host","localhost")
            ollama_port = ollama_settings.get("port",11434)
            client = ollama.Client(host=f'http://{ollama_host}:{ollama_port}')
            print(ollama_settings)

        question = q.iloc[0, 0]
        print(question)
        response = client.chat(model=ollama_settings["model"], messages=[
            {
                'role': 'user',
                'content': question
            },
        ...
        locals_added['qResult'] = response['message']['content']
        #locals_added['qResult'] = "Coucou!" + q.iloc[0, 0]
        print(locals_added['qResult'])
